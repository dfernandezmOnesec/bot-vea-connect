"""
Blob Trigger Processor Azure Function.

This function is triggered when a file is uploaded to Azure Blob Storage.
It processes the document by extracting text, generating embeddings, and storing them in Redis.
"""
...
def main(myblob: func.InputStream) -> None:
    """
    Processes uploaded documents from Blob Storage.
    
    This function:
    1. Downloads the uploaded file
    2. Extracts text based on file type (PDF, Word, Image, Text)
    3. Generates embeddings using OpenAI
    4. Stores embeddings and metadata in Redis
    5. Updates blob metadata to mark as processed
    
    Args:
        myblob (func.InputStream): The blob input stream containing the uploaded file.
    ...
    try:
        ...
        try:
            ...
            extracted_text = extract_text_from_file(temp_file_path, blob_name)
        ...
        error_message = f"Failed to process blob {blob_name}: {str(e)}"
        logger.error(error_message)
        raise

def extract_text_from_file(file_path: str, blob_name: str) -> str:
    """
